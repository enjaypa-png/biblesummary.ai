/**
 * Full New Testament Run — Sequential book-level execution
 *
 * For each book (Matthew → Revelation):
 *   Phase 1 rewrite → Phase 2 audit → Phase 3 auto-correction → Apply
 *
 * Logs per-book summary to ct-full-run-nt-log.jsonl.
 * Pauses if escalations > 5% of verses in a book.
 *
 * Usage:
 *   npm run ct:audit:full:run:nt
 *   npm run ct:audit:full:run:nt -- --estimate  # Cost estimate only, do not run
 */
...
const LOG_PATH = path.join(process.cwd(), 'data', 'translations', 'ct-full-run-nt-log.jsonl');
...
async function runBook(
  bookName: string
): Promise<{ totalVerses: number; phase2Pass: number; phase3Corrected: number; escalations: number; errors: number } | null> {
...
async function main() {
  ...
  if (!books?.length) {
    console.error('❌ No New Testament books found.');
    process.exit(1);
  }
  ...
  if (fromBook) {
    ...
    if (startIndex === -1) {
      console.error(`❌ from-book "${fromBook}" not found in New Testament books.`);
      process.exit(1);
    }
  ...
  console.log('New Testament Full Run — Cost Estimate');
  ...
  if (estimateOnly) {
    console.log('\n  Run without --estimate to start this NT segment.');
    return;
  }
  ...
  if (fromBook) {
    ...
    console.log('\n  Starting full New Testament run...\n');
  }
